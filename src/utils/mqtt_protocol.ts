...
// ==================== 2.2.17 MapClickInfo / 2.2.18 MapClickCmd ====================
...
export interface MapClickCmd extends Omit<MapClickInfo, 'mapY'> {}
...
const PASCAL_TO_MESSAGE_TYPE: Record<string, MessageType> = {
  ...
  MapClickInfo: 'map_click_info',
  MapClickCmd: 'map_click_cmd',
...
export function convertProtobufData(type: MessageType, decoded: any): AnyMessage | null {
  try {
    switch (type) {
      ...
      case 'map_click_info':
        return convertMapClickInfo(decoded)
...
function convertGameStatus(decoded: any): GameStatus {
  return {
    ...
    gameResult: Number(decoded.gameResult ?? decoded.game_result ?? 0) as GameResult,
    endReason: Number(decoded.endReason ?? decoded.end_reason ?? 255) as GameEndReason
  }
}

function convertMapClickInfo(decoded: any): MapClickInfo {
  return {
    isSendAll: Number(decoded.isSendAll ?? decoded.is_send_all ?? 0) as MapClickSendMode,
    robotId: decoded.robotId ?? decoded.robot_id ?? new Uint8Array(),
    mode: Number(decoded.mode ?? 0) as MapClickMode,
    enemyId: Number(decoded.enemyId ?? decoded.enemy_id ?? 0),
    ascii: Number(decoded.ascii ?? 0),
    type: Number(decoded.type ?? 0) as MapClickMarkType,
    mapX: Number(decoded.mapX ?? decoded.map_x ?? 0),
    mapY: Number(decoded.mapY ?? decoded.map_y ?? 0)
  }
}
...
function convertSentinelStatusSync(decoded: any): SentinelStatusSync {
  return {
    ...
    isPowered: Boolean(decoded.isPowered ?? decoded.is_powered ?? false)
  }
}
