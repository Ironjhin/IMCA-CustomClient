...
async function startUdpStream(): Promise<void> {
  ...
  try {
    ...
    const result = await window.api.udpStream.start({
      ...
      scaleHeight: udpConfig.value.scaleHeight ?? 1080
    })
 ...
 * 切换输出分辨率（自动重启流）
 */
async function handleResolutionChange(): Promise<void> {
  try {
    await window.api.saveSetting('udp.scaleHeight', udpConfig.value.scaleHeight)
  } catch (e) {
    console.warn('[UDP] Failed to persist scaleHeight:', e)
  }

  if (udpStreamStatus.value === 'connected') {
    console.log(`[UDP] Resolution changed to ${udpConfig.value.scaleHeight}, restarting stream...`)
    videoStatus.value = '切换分辨率中...'
    await stopUdpStream()
...
function handleMjpegStreamError(event: Event): void {
  const img = event.target as HTMLImageElement | null
  console.error('[MJPEG] stream load failed', { src: img?.src })
}
...
onMounted(async () => {
  ...
  // 加载持久化的分辨率设置
  try {
    const savedScaleHeight = await window.api.getSetting('udp.scaleHeight')
    if (savedScaleHeight !== null && savedScaleHeight !== undefined) {
      udpConfig.value = {
        ...udpConfig.value,
        scaleHeight: Number(savedScaleHeight)
      }
    }
  } catch (e) {
    console.warn('[UDP] Failed to load persisted scaleHeight:', e)
  }
            ...
            <div v-if="activeTab === 'network'" class="tab-panel">
              ...
              <div class="panel-section">
                ...
                <div class="form-row-group">
                  ...
                  <div class="form-item flex-1">
                    <label for="udp-resolution">输出分辨率</label>
                    <select
                      id="udp-resolution"
                      v-model.number="udpConfig.scaleHeight"
                      class="resolution-select"
                      @change="handleResolutionChange"
                    >
                      <option :value="480">480p</option>
                      <option :value="720">720p</option>
                      <option :value="1080">1080p (默认)</option>
