...
use crate::video_decoder::{SmartDecoder, YuvFrame, MjpegDecoder, JpegFrame};
...
pub struct UdpConfig {
    ...
    #[serde(default)]
    pub scale_height: Option<u32>, // GPU output scale height (0 = no scaling).
}
...
/// The UI can override this. 16 MiB gives enough headroom to absorb the large
/// IDR-frame bursts of a 1080p HEVC stream without the kernel dropping fragments
/// (a single dropped fragment loses the whole frame and triggers a multi-second
/// "Could not find ref" freeze until the next keyframe). The kernel clamps this
/// to net.core.rmem_max, so the effective size may be smaller.
...
///
/// This is the DEFAULT depth; override at runtime with `SHARK_DECODE_QUEUE`.
/// Trade-off: larger = fewer "drop queue + wait for IDR" stalls, but more
/// buffering latency (depth / fps seconds of glass-to-glass delay). Smaller =
/// lower latency, but a transient decode slowdown can stall on short-GOP streams.
const H265_DECODE_QUEUE_CAPACITY: usize = 8;

/// Runtime-tunable decode queue depth (`SHARK_DECODE_QUEUE`, default 8).
fn decode_queue_capacity() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        std::env::var("SHARK_DECODE_QUEUE")
            .ok()
            .and_then(|v| v.trim().parse::<usize>().ok())
            .filter(|&v| v >= 1 && v <= 240)
            .unwrap_or(H265_DECODE_QUEUE_CAPACITY)
    })
}

/// Runtime-tunable publish FPS cap (`SHARK_MAX_FPS`, default `MJPEG_FALLBACK_MAX_FPS`).
fn publish_max_fps() -> u32 {
    use std::sync::OnceLock;
    static FPS: OnceLock<u32> = OnceLock::new();
    *FPS.get_or_init(|| {
        std::env::var("SHARK_MAX_FPS")
            .ok()
            .and_then(|v| v.trim().parse::<u32>().ok())
            .filter(|&v| v >= 1 && v <= 240)
            .unwrap_or(MJPEG_FALLBACK_MAX_FPS)
    })
}
...
/// overwrites it; the encode thread consumes it. Keeping only the latest frame
/// (rather than a queue) means a slow CPU JPEG encode never backs up onto the
/// decode path — stale decoded frames are simply dropped, which is safe because
/// a decoded frame has no inter-frame (HEVC reference) dependencies.
struct DecodedSlot {
...
struct H265Worker {
    ...
    decoded: Arc<(StdMutex<DecodedSlot>, Condvar)>,
...
pub async fn udp_stream_start(
    ...
    // Use the larger of the UI-configured value and the compiled-in default.
    // A buffer that is too small causes kernel packet drops on IDR-frame bursts;
    // each dropped fragment loses the whole frame and triggers a multi-second
    // "Could not find ref" freeze until the next keyframe — much worse latency
    // than the few extra milliseconds of kernel buffering a larger buffer adds.
    let requested_recv_buffer_size = DEFAULT_RECV_BUFFER_SIZE.max(config.buffer_size);
    ...
    let jpeg_quality = std::env::var("SHARK_JPEG_QUALITY")
        .ok()
        .and_then(|v| v.trim().parse::<u8>().ok())
        .filter(|&v| v >= 1 && v <= 100)
        .or(config.jpeg_quality)
        .unwrap_or(MJPEG_FALLBACK_QUALITY);
    let scale_height = config.scale_height.unwrap_or(1080);
...
async fn udp_receive_loop(
    ...
    // One-shot diagnostic: log the leading bytes + detected type of the first
    // few assembled frames so we can confirm the on-wire payload is Annex B
    // HEVC and that IDR keyframes are actually being recognized.
    let mut diag_frames_logged: u32 = 0;
    ...
    // Latest-decoded-frame slot decouples the slow CPU JPEG encode from the
    // fast GPU decode. The decode thread never blocks on encoding, so the
    // compressed-frame queue never backs up and we never hit the
    // "queue full → drop + wait for keyframe" stall (the ~1 Hz GOP stutter).
    let decoded_slot: Arc<(StdMutex<DecodedSlot>, Condvar)> =
        Arc::new((StdMutex::new(DecodedSlot { frame: None }), Condvar::new()));
    let mut h265_worker: Option<H265Worker> = if output_codec != "h265" {
        // ---- Decode thread: H.265 → YUV, store latest, never blocks on encode ----
        let decode_handle = {
            let slot = Arc::clone(&h265_slot);
            let decoded = Arc::clone(&decoded_slot);
            let alive = Arc::clone(&h265_alive);
            let stats_w = Arc::clone(&stats);
            let mjpeg_d = mjpeg_broadcaster.clone();
            let target_scale_h = scale_height;
            std::thread::Builder::new()
                .name("udp-h265-decoder".into())
                .spawn(move || {
                    // Try MJPEG decoder first (FFmpeg outputs JPEG directly,
                    // no Rust-side YUV→RGB or JPEG encoding needed).
                    ...
                    while alive.load(Ordering::Relaxed) {
                        ...
                        // Slow path: SmartDecoder → YUV → to_jpeg() → publish.
                        let d = smart_dec.get_or_insert_with(SmartDecoder::new);
                        let decode_result = d.decode_to_yuv(&pending.data);
                        stats_w.set_decoder_backend(d.backend_name());
                        match decode_result {
                            Ok(Some(frame)) => {
                                // Publish to the latest-frame slot (overwrite).
                                let (lock, cvar) = &*decoded;
                                let mut guard = lock.lock().unwrap();
                ...
                .expect("spawn h265 decode thread")
        };

        // ---- Encode thread: latest YUV → JPEG → publish, paced to max FPS ----
        let encode_handle = {
            let decoded = Arc::clone(&decoded_slot);
            let alive = Arc::clone(&h265_alive);
            ...
            let frame_interval =
                Duration::from_millis((1000 / publish_max_fps().max(1)) as u64);
            std::thread::Builder::new()
                .name("udp-h265-encoder".into())
                .spawn(move || {
                    let mut last_publish = Instant::now()
                        .checked_sub(frame_interval)
                        .unwrap_or_else(Instant::now);
                    while alive.load(Ordering::Relaxed) {
                        // Take the freshest decoded frame, waiting if none yet.
                        let frame_opt = {
                            let (lock, cvar) = &*decoded;
                            let mut guard = lock.lock().unwrap();
                            while guard.frame.is_none() && alive.load(Ordering::Relaxed) {
                                guard = cvar.wait(guard).unwrap();
                            }
                            guard.frame.take()
                        };
                        ...
                        // Pace to the FPS cap. If the freshest frame arrived too
                        // soon after the last publish, drop it (a newer one will
                        // come). This bounds CPU/bandwidth without adding latency.
                        let now = Instant::now();
                        ...
                        if let Some(mjpeg) = &mjpeg_w {
                            ...
                            // Non-MJPEG fallback: compact JPEG over the IPC channel.
                            send_channel_frame(
                                &ch_w,
                                &stats_w,
                                FRAME_TYPE_JPEG,
                ...
                .expect("spawn h265 encode thread")
        };

        Some(H265Worker {
            alive: h265_alive.clone(),
            slot: h265_slot.clone(),
            decoded: decoded_slot.clone(),
                ...
                match res {
                    Ok((len, addr)) => {
                        ...
                        if fragment_index == 0 && payload.len() as u32 == total_frame_size {
                            ...
                            if diag_frames_logged < 8 {
                                diag_frames_logged += 1;
                                let head: Vec<String> =
                                    payload.iter().take(16).map(|b| format!("{:02x}", b)).collect();
                                info!(
                                    seq = diag_frames_logged,
                                    out_codec = %output_codec,
                                    detected = %info.r#type,
                                    is_keyframe = info.is_keyframe,
                                    nal_type = ?info.nal_type,
                                    len = payload.len(),
                                    head = %head.join(" "),
                                    "DIAG single-packet frame"
                                );
                            }
                        ...
                        if complete {
                            ...
                            if let Some(finished) = in_flight_frames.remove(&frame_number) {
                                ...
                                if diag_frames_logged < 8 {
                                    diag_frames_logged += 1;
                                    let p = finished.payload();
                                    let head: Vec<String> =
                                        p.iter().take(16).map(|b| format!("{:02x}", b)).collect();
                                    info!(
                                        seq = diag_frames_logged,
                                        out_codec = %output_codec,
                                        detected = %info.r#type,
...
fn send_frame_to_frontend(
    ...
    if info.r#type == "h265" && output_codec != "h265" {
        ...
        if guard.frames.len() >= decode_queue_capacity() {
            if info.is_keyframe {
