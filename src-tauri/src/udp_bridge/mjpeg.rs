...
pub(super) const MJPEG_FALLBACK_QUALITY: u8 = 50;
pub(super) const MJPEG_FALLBACK_MAX_FPS: u32 = 60;
...
fn serve_mjpeg_client(
    ...
    tracing::debug!("MJPEG client connected");
    let _ = stream.set_read_timeout(Some(Duration::from_millis(100)));
    let _ = stream.set_write_timeout(Some(Duration::from_secs(2)));

    // Read the HTTP request so we can echo back the Origin header. WebKit's
    // CORS check for multipart/x-mixed-replace rejects a bare `*` wildcard in
    // some versions; echoing the exact Origin is the reliable path.
    ...
    let origin = request
        ...
        .find_map(|line| {
            ...
            if lower.starts_with("origin:") {
                Some(line["origin:".len()..].trim().to_string())
            } else {
                None
            }
        })
