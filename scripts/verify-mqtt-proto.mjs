...
const v2Fields = {
  GameStatus: ['gameResult', 'endReason'],
  MapClickInfo: ['isSendAll', 'robotId', 'mode', 'enemyId', 'ascii', 'type', 'mapX', 'mapY'],
  MapClickCmd: ['isSendAll', 'robotId', 'mode', 'enemyId', 'ascii', 'type', 'mapX'],
  SentryStatusSync: ['isPowered'],
};
for (const [messageName, fields] of Object.entries(v2Fields)) {
  const type = root.lookupType(messageName);
  const missingFields = fields.filter((field) => !type.fields[field]);
  if (missingFields.length) throw new Error(`${messageName} missing fields: ${missingFields.join(', ')}`);
}
if (root.lookupType('MapClickCmd').fields.mapY) {
  throw new Error('MapClickCmd must not contain mapY');
}
console.log('[v2] Required V2.0.0 messages and fields present ✅');
